...
if [ -d "$HUMANIZER" ]; then
  ...
  run "git -C '$HUMANIZER' pull --ff-only"
...
PYEOF
  ...
  echo ""
  echo "    open ${CLAUDE_DIR}/claude-local-starter.html"
  echo ""
...
if [ -d "$CMD_SRC" ]; then
  ...
  run "cp -rn '$CMD_SRC/.' '${CLAUDE_DIR}/commands/'"  # -n: no overwrite
...
if [ -d "$SKILLS_SRC" ] && [ "$(ls -A "$SKILLS_SRC" 2>/dev/null)" ]; then
  ...
  run "cp -rn '$SKILLS_SRC/.' '${CLAUDE_DIR}/skills/'"
...
[ "${BASH_SOURCE[0]}" != "$SCRIPT_DST" ] && {
  run "cp '${BASH_SOURCE[0]}' '$SCRIPT_DST'"
